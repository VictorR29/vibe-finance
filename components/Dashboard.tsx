...
import { ArrowUpRight, ArrowDownLeft, Target, Wallet, Globe, Calendar, PiggyBank } from 'lucide-react';
import { cn } from '../utils/cn';
import { SavingsGoal } from '../types';


const SavingsGoalProgress: React.FC<{ goal: SavingsGoal, currency: string }> = ({ goal, currency }) => {
    ...
    return (
        <div className="space-y-2 p-3 rounded-xl hover:bg-white/10 transition-colors">
            <div className="flex justify-between items-center text-sm">
                ...
                <span className="text-primary font-bold">{formatCurrency(goal.targetAmount, currency)}</span>
            </div>
...
export const Dashboard: React.FC = () => {
    ...
    const expenseByCategory = useMemo(() => {
        ...
        return Array.from(categoryMap.entries()).map(([name, value]) => ({ name, value })).sort((a,b) => b.value - a.value).slice(0, 5);
    }, [state.transactions]);
    ...
    return (
        <div className="space-y-6 pb-20 md:pb-0"> 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                <CardStat label="Total Ingresos (Histórico)" value={formatCurrency(globalStats.income, state.currency)} icon={<ArrowUpRight className="w-6 h-6"/>} color="bg-emerald-500" />
                <CardStat label="Total Gastos (Histórico)" value={formatCurrency(globalStats.expense, state.currency)} icon={<ArrowDownLeft className="w-6 h-6"/>} color="bg-rose-500" />
                <CardStat label="Balance Global" value={formatCurrency(globalStats.balance, state.currency)} icon={<Wallet className="w-6 h-6"/>} color="bg-primary" />
            </div>
            ...
            <Card className="overflow-visible">
                        ...
                        <div className="flex bg-white dark:bg-gray-800 rounded-lg p-0.5 shadow-sm">
                            <button 
                                ...
                                className={cn(
                                    ...
                                    filterType === 'monthly' ? "bg-primary text-white shadow" : "text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"
                                )}
                            >
                                Mensual
                            </button>
                            <button 
                                ...
                                className={cn(
                                    "px-3 py-1.5 text-xs font-medium rounded-md transition-all",
                                    filterType === 'yearly' ? "bg-primary text-white shadow" : "text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white"
                                )}
                            >
                                Anual
                            </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-500/20 flex flex-col items-center text-center">
                        <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1">Ingresos</span>
                        <span className="text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(periodStats.income, state.currency)}</span>
                    ...
                    <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-500/20 flex flex-col items-center text-center">
                        <span className="text-xs font-semibold text-rose-600 dark:text-rose-400 uppercase tracking-wider mb-1">Gastos</span>
                        <span className="text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(periodStats.expense, state.currency)}</span>
                        <div className="mt-2 w-full h-1 bg-rose-100 dark:bg-rose-900/30 rounded-full overflow-hidden">
                            ...
                            <div 
                                ...
                                style={{ width: `${periodStats.income > 0 ? Math.min((periodStats.expense / periodStats.income) * 100, 100) : 100}%` }}
                            ></div>
                        </div>
                    </div>
                     ...
                     <div className="p-4 rounded-2xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-500/20 flex flex-col items-center text-center relative overflow-hidden">
                        <span className="text-xs font-semibold text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-1">Resultado Neto</span>
                        <span className={cn(
                            "text-2xl font-bold",
                            periodStats.balance >= 0 ? "text-gray-900 dark:text-white" : "text-rose-600 dark:text-rose-400"
                        )}>
                            {periodStats.balance >= 0 ? '+' : ''}{formatCurrency(periodStats.balance, state.currency)}
                        </span>
                         <div className="mt-2 flex items-center gap-1 text-xs text-blue-600/70 dark:text-blue-400/70">
                            <PiggyBank className="w-3 h-3" />
                            <span>{periodStats.income > 0 ? Math.round(((periodStats.income - periodStats.expense) / periodStats.income) * 100) : 0}% margen</span>
                        </div>
                    </div>
                </div>
            </Card>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 <Card title="Top Gastos (Histórico)" className="lg:col-span-2 min-h-[400px]">
                    <div className="h-[300px] w-full mt-4 min-w-0">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={expenseByCategory} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barSize={40}>
                                <XAxis dataKey="name" tick={{ fontSize: 12, fill: tickColor }} axisLine={false} tickLine={false} dy={10} />
                                <YAxis 
                                    width={80} 
                                    tickFormatter={(value) => formatCurrency(value, state.currency).replace(/(\.00|,\d+)/, '')} 
                ...
                <Card title="Metas de Ahorro">
                    <div className="space-y-4 mt-2">
                        {sortedGoals.length > 0 ? sortedGoals.slice(0, 4).map(goal => (
                            ...
                            <div className="flex flex-col items-center justify-center h-[300px] text-center p-4">
                                ...
                                <h3 className="text-lg font-medium text-gray-900 dark:text-white">Sin metas activas</h3>
                                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">Define objetivos financieros para ver tu progreso aquí.</p>
                            </div>
                        )}
                    </div>
                </Card>

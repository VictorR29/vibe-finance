  ...
  return (
    <div className="relative overflow-hidden bg-white/60 dark:bg-gray-900/60 backdrop-blur-lg border border-white/30 dark:border-white/10 p-5 rounded-3xl shadow-lg group hover:-translate-y-1 transition-transform duration-300">
      <div className="flex items-center justify-between relative z-10">
        ...
        <div
          className={`p-3 rounded-2xl ${styles.bg} ${styles.text} shadow-inner group-hover:scale-110 transition-transform duration-300 backdrop-blur-md border border-white/10`}
        >
          {icon}
        </div>
      </div>
      ...
      <div
        className={`absolute -right-6 -bottom-6 w-24 h-24 rounded-full ${styles.glow} opacity-20 dark:opacity-40 blur-2xl group-hover:opacity-30 dark:group-hover:opacity-60 transition-opacity`}
      ></div>
    </div>
  );
};
